...
def test_swimlane_widget_renders():
    ...
    assert any(ch in lane for ch in ("●", "◇", "o", "+", "*"))  # should contain session markers
...
def test_swimlane_sets_explicit_height_after_data():
    """Swimlane should not rely on auto height after late content updates."""
    import asyncio
    ...
    class SwimlaneApp(App):
        CSS = "Swimlane { width: 62; height: auto; }"
    ...
    async def run_case() -> None:
        ...
        async with app.run_test() as pilot:
            widget = app.query_one(Swimlane)
            widget.set_data(cluster, compact=True)
            await pilot.pause(0.3)
            assert str(widget.styles.height) != "auto"
            assert widget.size.height > 1

    asyncio.run(run_case())


def test_toggle_to_swimlane_resets_scroll_position():
    """Switching to swimlane should reset viewport to the top."""
    import asyncio
    from pathlib import Path
    ...
    sessions = [
        SessionInfo(
            ...
            project_dir="GUI",
            jsonl_path=Path(f"/tmp/s{i:04d}.jsonl"),
            first_timestamp=now,
            last_timestamp=now,
            message_count=10,
            status=Status.DONE,
    ...
    wf = Workflow(
        ...
        name="workflow one",
        root_session_id=sessions[0].session_id,
        first_timestamp=now,
        last_timestamp=now,
    )
    cluster = WorkflowCluster(worktree="main", project="GUI", workflows=[wf], orphans=[])

    class SessionListApp(App):
        CSS = "SessionListPanel { width: 80; height: 12; }"
    ...
    async def run_case() -> None:
        ...
        async with app.run_test() as pilot:
            ...
            panel.load_sessions(sessions, workflow_cluster=cluster)
