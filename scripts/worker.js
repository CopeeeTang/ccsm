...
try {
  if (action === 'index-refresh') {
    execSync(
      `${pythonCmd} -c "from ccsm.core.index_db import incremental_refresh; count = incremental_refresh(); print(f'CCSM: refreshed {count} sessions')"`,
      {
        cwd: pluginRoot,
    ...
    // Read sessionId from stdin if available, otherwise just refresh index
    execSync(
