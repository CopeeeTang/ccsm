...
@dataclass
class SessionDigest:
    """Five-dimension structured AI summary — the Detail panel's first screen.

    Each dimension answers one recovery question:
      goal:       What was I trying to do?
      progress:   How far did I get?
      breakpoint: Where exactly did I stop?
      next_steps: What should I do next?
      blocker:    Is anything blocking me?

    Generated by LLM from full user messages + compact summary + milestones.
    Inspired by Claude-mem's five-dimension session summary pattern.
    """

    goal: str                                   # User's original objective (one sentence)
    progress: str                               # What was accomplished (2-3 sentences)
    breakpoint: str                             # Exact stopping point (one sentence)
    next_steps: list[str] = field(default_factory=list)  # 1-3 concrete action items
    blocker: Optional[str] = None               # What's blocking, or None
