...
class SessionSummary:
    ...
    # Phase 1 additions: AI Digest + Facts (optional, backward-compatible)
...
@dataclass
class SessionDigest:
    """Four-section structured Digest — the Detail panel's core display.

    Each section serves a distinct recovery purpose:
      progress:   What was accomplished? (2-3 sentences, specific)
      decisions:  What key decisions were made? (2-5 items with rationale)
      breakpoint: Where exactly did I stop? (most important, visually prominent)
      todo:       What should I do next? (1-3 concrete action items)

    Note: No goal/intent field — Title already covers "what the session is about".
    Generated by LLM from full user messages + compact summary + milestones.
    """

    progress: str                               # What was accomplished (2-3 sentences)
    breakpoint: str                             # Exact stopping point (visually prominent)
    decisions: list[str] = field(default_factory=list)  # 2-5 key decisions with rationale
    todo: list[str] = field(default_factory=list)  # 1-3 concrete action items
    # Legacy compat: safe defaults to avoid None crashes in existing consumers
    goal: str = ""                              # Deprecated → Title covers this
    next_steps: list[str] = field(default_factory=list)  # Deprecated → use todo
    blocker: Optional[str] = None               # Deprecated → merged into breakpoint


@dataclass
class SessionFact:
    """An atomic, self-contained fact extracted from a session.

    Each fact is independently searchable and understandable without
    context from the surrounding conversation. Inspired by Mem0's
    atomic fact extraction pattern.

    Fact types:
      decision   — A choice made (e.g., "chose Drawer over Tab layout")
      discovery  — Something learned (e.g., "compact summary has 97.9% coverage")
      config     — A configuration/setup (e.g., "API proxy at 52.151.57.21:9999")
      constraint — A limitation found (e.g., "Textual Collapsible needs CSS fix")
    """

    content: str                                # The fact statement
    fact_type: Optional[str] = None             # decision/discovery/config/constraint
    source: Optional[str] = None                # Where (milestone label or context)
